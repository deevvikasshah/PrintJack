...
export default function SmokeCursor() {
  ...
  useEffect(() => {
    ...
    const contextParams = { alpha: true, depth: false, stencil: false, antialias: false, preserveDrawingBuffer: false };
    let gl = canvas.getContext('webgl2', contextParams);
    const isWebGL2 = !!gl;
    if (!isWebGL2)
      gl = canvas.getContext('webgl', contextParams) || canvas.getContext('experimental-webgl', contextParams);
    ...
    function supportRenderTextureFormat(context, internalFormat, format, type) {
      ...
      if (status !== context.FRAMEBUFFER_COMPLETE) return false;
      return true;
    }
  ...
  return <canvas ref={canvasRef} className="tcg-smoke-cursor" aria-hidden="true" />;
}
